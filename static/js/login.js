(() => {
  ...
  function applyBlink(eyeEl, blinking) {
    ...
    const pupil = eyeEl.querySelector('.pupil');
    pupil.style.display = blinking ? 'none' : 'block';
  }
  ...
  // continuous small loop so blink/typing/peek state changes also reflect visually
  ...
  function setLoading(loading) {
    ...
    const text = loading ? 'Authenticating...' : 'Log in';
    submitLabel.textContent = text;
    submitLabelHover.textContent = text;
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (roleErr || emailErr || passErr) {
      formError.textContent = roleErr || emailErr || passErr;
    ...
    try {
      ...
      if (data.success) {
        ...
        formError.textContent = data.message || 'Logged in successfully! Redirecting...';

...
def calculate_correlation(df, semantic_types, method="pearson"):
    """
    Calculates correlation matrix strictly for measure, currency, and percentage columns.
    ...
    non_constant_cols = [col for col in numeric_df.columns if numeric_df[col].dropna().nunique() > 1]

    if len(non_constant_cols) < 2:
        logger.debug("Not enough variable numeric columns for correlation analysis.")
        return None

    clean_data = numeric_df[non_constant_cols].dropna()

    if len(clean_data) < 3:
        logger.debug("Not enough data points for meaningful correlation after dropping NA.")
        return {
            "matrix": {},
            "p_values": {},
            "top_pairs": []
    ...
    for i in range(len(cols)):
        for j in range(len(cols)):
            ...
            if i == j:
                ...
                # Ensure both columns have variance before calculating p-value
                if clean_data[col1].std() == 0 or clean_data[col2].std() == 0:
                    r_val = corr_df.loc[col1, col2]
                    p_val = 1.0  # Not significant if one is constant
                else:
                    try:
                        if HAS_SCIPY:
                            if method == "spearman":
                                r_val, p_val = stats.spearmanr(clean_data[col1], clean_data[col2])
                            elif method == "kendall":
                                r_val, p_val = stats.kendalltau(clean_data[col1], clean_data[col2])
                            else:

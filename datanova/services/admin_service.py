...
def get_admin_dashboard_analytics(conn=None) -> Dict[str, Any]:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # Storage Used (in GB and percentage of 500 GB threshold)
            ...
            # Weekly Activity Breakdown (Actual Last 7 Days counts).
            # Previously this ran up to 3 queries PER day (21 queries total) in a
            # Python loop. Grouped into 2 queries covering the whole week — same
            # result, far fewer round trips, and no per-day try/except silently
            # zeroing a day out if one query in the loop failed.
            ...
            for i in range(7):
                ...
                weekly_active.append(success_by_date.get(day, 0) + dataset_by_date.get(day, 0))
                weekly_inactive.append(fail_by_date.get(day, 0))

            kpi_data['weekly_activity'] = {
                'labels': weekly_labels,
                'active': weekly_active,
                'inactive': weekly_inactive
            }
